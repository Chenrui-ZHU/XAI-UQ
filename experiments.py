...
def robustness_test():
    ...
    # Lists to collect uncertainty and un-robustness for each experiment
    ...
    for _ in range(n_iterations):
        ...
        # List to hold the un-robustness value for each test instance
        unrobust_values = []
        ...
        for i, x in enumerate(X_test):
            ...
            # The un-robustness measure for x_i is the maximum ratio over its neighborhood
            unrobust_values.append(np.max(ratios))
        
        unrobust_values = np.array(unrobust_values)
        
        # Compute uncertainty (Shannon entropy on prediction probabilities)
        probas = model.predict_proba(X_test)
        uncertainties = entropy(probas, base=2, axis=1)
        
        # Append the uncertainties and un-robustness values to the global lists
    ...
    # Create a combined 2D array for sorting by uncertainty (first column: uncertainty, second: un-robustness)
    ...
    plt.savefig("figures/robustness_vs_uncertainty_wine.png")